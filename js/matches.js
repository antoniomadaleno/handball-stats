  ...
  Promise.all([DB.matches.bySeason(S.season.id), DB.opponents.bySeason(S.season.id)]).then(([matches, opps]) => {
    ...
    el.innerHTML = matches.map(m => {
      ...
      return `<div class="match-card">
        ...
        <div style="flex:1">
          <div class="match-vs">${home} <span style="color:var(--text3)">vs</span> ${away}${m.type === 'scout' ? ' <span class="badge b-scout">Scout</span>' : ''}</div>
        ...
        <div style="display:flex;gap:6px" onclick="event.stopPropagation()">
          ...
          <button class="btn btn-primary btn-sm" onclick="app.openMatchEvents(${m.id})">Abrir</button>
...
export function openMatchEvents(id) {
  ...
  alert('Recolha de eventos — em breve!');
}